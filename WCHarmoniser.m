...
r = coef/3*abs(rand(1)); % random number to get randomization to transpose
...
winlen = round(Fs/f); % window length
...
for n = 2:length(x)
    for k = 1:4
        ...
        if abs(p(n,k) - p(n-1,k)) > coef*0.8 % finding the peaks of phasor
            ...
            r = coef/3*abs(rand(1)); % new random for every window
